...
/// Set the direction of the door to open
...
/// Sets the default for the door direction to be opening to the side. This is a normal thing for doors to do.
...
/// A door is a node that can be opened and closed.
/// This struct defines nodes in the door to open and close:
/// - how light affects it
/// - how it interacts with other nodes
/// - what state it can be in (Opened, Opening, Closed, Closing, Locked, Broken)
/// - Its offset ()
/// - Its location in the world/Scene
/// - Which direction it opens in
/// - How fast it opens(i dont know, actually.)
...
impl Door {
    ...
    /// ## Simple explanation
    /// Sets the color that the lights make to the door
    ///
    /// ## What it does
    /// The ``for`` loop is used to iterate through the lights in the ``Door.lights`` vector and does the following for each iteration. The & symbol is used to make the light
    /// that is being iterated mutable, meaning it can change between iterations and so each light that affects the door can be changed by this function.
    /// 1. graph[light] makes sure that the engine knows that it is talking about the light node in the graph that the current iteration is on
    /// 2. .query_component_mut::<BaseLight>() gets the mutable values of the BaseLight strutct. Every node has a Base node and light is no exception
    /// 3. .set_color() sets the color of the light to the color value set when calling this function.
    /// - If you set the color value in the function call to ``color({0,0,0,0})`` then the light shaded to the door will be black.
    ...
    /// ## Simple explanation
    /// Sets the lights to affect the door
    ///
    /// ## What it does
    /// for each light node that gets iterated through in the Door::lights vector, it does the following
    /// graph[light] gets the light node in the scene graph to be targeted by the engine
    /// .set_visibility(enabled) presumably sets the light nodes to make the door visible
