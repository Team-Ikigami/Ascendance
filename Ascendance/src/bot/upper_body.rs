...
impl UpperBodyMachine {
	...
	// Appears in the RON files. These constants are defined with a static string so the ron files can be read. The constnats are used later in CODE.
    ...
    pub async fn new(
		...
		// Adds a bunch of transitions to be available.
		// Parameters of Machine::new().add_transition(Transition::new()) are:
			// name: a mutable string
			// src: Beginning animation of transition
			// dest: final animation of transition, the reason the transition happens
			// time: length of time it takes for the transition to finish
			// rule: a constant. Appears many times in the .ron config files.
		// very bloaty in size and LUCKILY will be dealt with beautiifully in the next Fyroxed update.
